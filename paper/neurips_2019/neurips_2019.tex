...
\usepackage{amsmath,amssymb,amsfonts,amsthm,mdframed,relsize}
\usepackage{graphicx}
\usepackage{bbm}
\usepackage{latexsym}
\usepackage{natbib}
...
a randomized response method $\mathcal{R}(\cdot)$ is $\epsilon$-private if for all pairs of observations $y, y'$\looseness=-1
\begin{equation}
...
should be made indistinguishable.
  ...
  \textbf{Locally private MCMC for Poisson factorization}. \swcomment{the following is not really about private MCMC; more about how we use prior work to set up the CAVI algo...} In locally
...
To infer LPBPF,\swcomment{we never define this term...} one may first infer the sums of Poisson distributions of the
...
This second factorization enables direct closed-form inference: unlike in
  ...
  \swcomment{the text above is a big wave of math, with several definitions missing (e.g. bessel, skellam). it's a bit hard to read through the text. the table from the previous paper might be useful for exposition (attached below in case it's useful)}
...
\end{figure*}
  ...
  \frac{1}{m!\Gamma(m\tp\nu\tp 1)}$.\swcomment{what's $\Gamma$?} \looseness=-1
  \end{proposition}
