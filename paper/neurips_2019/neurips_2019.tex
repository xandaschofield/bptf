...
\begin{abstract}
    ...
    infer Poisson factorization models over count data by first applying
    differentially private noise, then using Bayesian inference to estimate the
    true data priors given knowledge of the random noise distribution. While the
    existing work uses an MCMC algorithm, this sampling-based method can be slow
    for high-dimensional datasets such as those for language models or
    large-scale social networks. We improve on this method with a coordinate
    ascent variational inference (CAVI) algorithm to infer factorization models
    efficiently from private data. Our algorithm relies on several new properties we
    prove about Bessel distributions. Our algorithm produces a substantial speed
    improvement over a comparable MCMC algorithm in experiments on private Bayesian
    matrix factorization over synthetic and real data.
  \end{abstract}
...
This second factorization enables direct closed-form inference: unlike in
  ...
  \begin{figure}[t]
    \centering
    \includegraphics[width=0.4\textwidth]{figures/besselmode}
    \caption{Several comparisons of the mode versus the mean of the Bessel distribution. Each color represents a choice of $\nu$, with shapes of markers denoting the mode versus the mean.}
    \label{fig:besselmode}
  \end{figure}

  Fortunately, we empirically observe that these values are close.
  Figure \ref{fig:besselmode} shows that, though the mode is an integer-valued staircase distribution compared to the continuous mean, these quantities remain close across different parameter values of $\nu$ and $\alpha$.
  We can even provide a theoretical bound for the error between these two, which we prove in a
  third proposition:
  ...
  \begin{figure*}[t]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/test_output.pdf}
