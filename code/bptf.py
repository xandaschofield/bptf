...
class BPTF(BaseEstimator, TransformerMixin):
    ...
    def _update_theta_gamma(self, m):
        tmp_DIMS = self.y_E_DIMS.astype(float)
    ...
    def _update_lam_shp(self, mask=None):
        self.lam_shp_2DIMS[0] = self.y_pos_E_DIMS + 1
        self.lam_shp_2DIMS[1] = self.y_pos_E_DIMS - self.data_DIMS + 1
        if mask is not None:
    ...
    def _update_mins(self, mask=None):
        ...
        nu_DIMS = self.data_DIMS.dot(mask)
    ...
    def fit(self, data, priv, mask=None):
        ...
        self.data_is_positive_DIMS = (data > 0).astype(int)
    ...
    def y_pos_E_DIMS(self):
        return self.min_DIMS + (self.data_is_positive_DIMS * self.data_DIMS)

    @property
    def g_neg_E_DIMS(self):
        return self.min_DIMS + ((1 - self.data_is_positive_DIMS) * self.data_DIMS)
    ...
    def fit_transform(self, modes, data, priv, mask=None, version='geometric'):
        assert all(m in range(self.n_modes) for m in modes)
        assert (version == 'geometric') or (version == 'arithmetic')
